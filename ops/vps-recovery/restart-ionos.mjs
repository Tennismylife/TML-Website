...
async function waitForPassword(target=page,ms=10000){const end=Date.now()+ms;while(Date.now()<end){const p=await firstVisible(target.locator('input[type="password"]'));if(p)return p;for(const f of target.frames()){const q=await firstVisible(f.locator('input[type="password"]')).catch(()=>null);if(q)return q}await target.waitForTimeout(400)}return null}
...
async function fillTotpIfNeeded(target=page){const t=await body(target);if(!/verification code|authenticator|two[- ]factor|2fa|codice di verifica|security code|bestätigungscode/.test(t))return;if(!totpSecret)throw new Error('IONOS requested 2FA; add IONOS_TOTP_SECRET to GitHub Secrets');const otp=await firstVisible(target.locator('input[autocomplete="one-time-code"],input[inputmode="numeric"],input[type="tel"]'));if(!otp)throw new Error('IONOS 2FA field not found');await otp.fill(totp(totpSecret));if(!(await clickSubmit(target))&&!(await clickText([/^continue$/i,/^verify$/i,/^confirm$/i,/^bestätigen$/i,/^conferma$/i],target)))await otp.press('Enter');await target.waitForTimeout(2500)}
...
try{
 ...
 let p=await waitForPassword(page,10000); if(!p){await safeDiag(page);throw new Error('IONOS account password field not found after username step')}
 await p.fill(password); if(!(await clickSubmit(page))&&!(await clickText([/^next$/i,/^log\s*in$/i,/^sign\s*in$/i,/^anmelden$/i,/^accedi$/i])))await p.press('Enter');
 await page.waitForTimeout(3000); await fillTotpIfNeeded(page);
 let t=await body(page); if(/invalid|incorrect|wrong password|login failed|anmeldung fehlgeschlagen|credenziali.*non valide/.test(t)){await safeDiag(page);throw new Error('IONOS account login failed')}
 if(page.url().includes('login.ionos.com')&&await firstVisible(page.locator('input[type="password"]'))){await safeDiag(page);throw new Error('IONOS account login did not leave password page')}
 ...
 let opened=await clickText([/server\s*&\s*cloud/i,/servers\s*&\s*cloud/i]); if(!opened){await clickText([/^menu$/i,/menu/i]);await page.waitForTimeout(700);opened=await clickText([/server\s*&\s*cloud/i,/servers\s*&\s*cloud/i])}
 ...
 let server=await firstVisible(page.getByText(serverMatch,{exact:false}));if(!server){await clickText([/^servers$/i,/^server$/i,/infrastructure/i],page);await page.waitForTimeout(1800);server=await firstVisible(page.getByText(serverMatch,{exact:false}))}
 ...
 await server.click();await page.waitForTimeout(1200);if(!(await clickText([/^actions$/i,/^aktionen$/i,/^azioni$/i],page)))throw new Error('Actions menu not found');if(!(await clickText([/^restart$/i,/^reboot$/i,/^riavvia/i,/^neustart/i],page)))throw new Error('Restart action not found');await page.waitForTimeout(700);
 if(dryRun)console.log('DRY RUN OK: reached Restart confirmation; no reboot sent.');else{if(!(await clickText([/^yes$/i,/^ja$/i,/^sì$/i,/^si$/i,/^confirm$/i,/^conferma$/i],page)))throw new Error('Restart confirmation not found');console.log('IONOS restart request submitted.')}
