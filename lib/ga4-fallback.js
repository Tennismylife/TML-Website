...
async function handleGa4Fallback(req, res, options = {}) {
  ...
  try {
    ...
    // Lightweight receive log for debugging (no PII; page_path and truncated UA only)
    if (debugMode) {
      const shortUa = String(req.body?.user_agent || req.headers['user-agent'] || '').slice(0, 120);
      console.debug('[GA4-FALLBACK] received', { path: req.path, page_path: req.body?.page_path, ua: shortUa });
    }
    ...
    if (debugMode) {
      console.debug('[GA4-FALLBACK] debug_mode enabled; payload will include debug_mode');
    }
    ...
    try {
      ...
      if (process.env.GA4_FALLBACK_DEBUG === '1') {
        console.debug('[GA4-FALLBACK] forward_ok', { path: req.path, client_id: (client_id || '').slice(0,8) });
      }
      ...
      // Diagnostic logs: show error message and (optionally) full stack when debug enabled
      console.error('[GA4-FALLBACK ERROR] forward failed:', err && err.message);
      if (process.env.GA4_FALLBACK_DEBUG === '1') {
        console.error(err);
        console.debug('[GA4-FALLBACK] forward_fail', { path: req.path, err: err && err.message });
      }
...
async function serveGif(req, res) {
  try {
    ...
    if (debugMode) {
      console.debug('[GA4-FALLBACK] debug_mode enabled for GIF beacon');
    }
