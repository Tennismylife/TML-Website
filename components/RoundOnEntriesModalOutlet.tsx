...
export default function RoundOnEntriesModalOutlet({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    const openWithPayload = (detail: any) => {
      ...
      const url = titleParam ? `/api/tournaments/${id}/records/roundsonentries?full=true&round=${encodeURIComponent(String(titleParam))}` : `/api/tournaments/${id}/records/roundsonentries?full=true`;
      fetch(url)
    ...
    try {
      ...
      if (last && last.section === 'roundsonentries') {
        openWithPayload(last);
        try { delete (window as any).__lastOpenModalPayload; } catch (e) {}
      }
    } catch (e) {}

    if (isModal && state?.section === 'roundsonentries') {
    ...
    const handleOpenModal = (e: any) => {
      ...
      if (!detail) return;
      if (detail.section !== 'roundsonentries') return;
    ...
    <RouteModal>
      <div className="text-white">
        <div className="mb-3 text-center">
          {(() => {
            if (!title) return <h3 className="text-2xl font-semibold">Round on Entries at {tourneyName}</h3>;
            const normalized = String(title || '').trim();
            const displayTitle = normalized.toLowerCase() === 'winner' ? 'Titles' : `${normalized}s`;
            return <h3 className="text-2xl font-semibold">Most {displayTitle} on Entries at {tourneyName}</h3>;
          })()}
        </div>
