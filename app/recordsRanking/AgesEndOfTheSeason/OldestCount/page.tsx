...
export default function OldestEoyAtRank() {
  const [rank, setRank] = useState<number>(1);
  const [rows, setRows] = useState<OldestEoyItem[]>([]);
  ...
  const fetchRows = async (selectedRank: number) => {
    ...
    try {
      const res = await fetch(
        `/api/recordsranking/agesendoftheseason/oldestcount?rank=${selectedRank}&limit=200`
      );
      const data = await res.json();
      setRows(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error("Error fetching oldest EOY:", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderTable = (list: OldestEoyItem[], startIndex = 0) => (
  ...
  return (
    <section className="mb-8">
      ...
      <h2 className="text-xl font-semibold mb-4 text-gray-200 text-center">
        Oldest Players at Year-End No. {rank}
      </h2>
