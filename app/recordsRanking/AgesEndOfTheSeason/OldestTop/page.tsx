...
export default function OldestEoyTopX() {
  const [top, setTop] = useState<number>(2); // es. Top 2
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/recordsranking/agesendoftheseason/oldesttop?top=${top}&limit=200`);
      const data: OldestEoyTopItem[] = await res.json();
