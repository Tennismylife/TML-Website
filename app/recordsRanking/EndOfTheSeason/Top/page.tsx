...
interface PlayerTopX {
  ...
  name: string;
  ioc?: string | null;
  endYearTopCount: number; // numero stagioni in Top X
  seasons: number[];       // elenco anni semplici
}

export default function RecordsTopX() {
  const [players, setPlayers] = useState<PlayerTopX[]>([]);
  const [top, setTop] = useState<number>(2); // default Top 2 come da esempio
  ...
  const fetchPlayers = async (selectedTop: number) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/recordsranking/endofseason/top?top=${selectedTop}`);
  ...
  useEffect(() => {
    fetchPlayers(top);
    setPage(1); // reset pagina quando cambia Top X
  }, [top]);
  ...
  const renderTable = (list: PlayerTopX[], startIndex = 0) => (
  ...
  return (
    <section className="mb-8">
      ...
      {/* Modal View All */}
      <Modal show={showModal} onClose={() => setShowModal(false)} title={`Seasons in Top ${top}`}>
        {renderTable(players)}
      </Modal>
    </section>
  );
