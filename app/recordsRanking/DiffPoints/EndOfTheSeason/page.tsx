...
export default function YearEndDifferenceNo1No2() {
  const [rows, setRows] = useState<YearEndItem[]>([]);
  ...
  const fetchRows = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/recordsranking/diffpoints/endoftheseason`, { cache: "no-store" });
      const data: YearEndItem[] = await res.json();
      setRows(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error("Error fetching year-end differences:", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderTable = (list: YearEndItem[]) => (
    <div className="overflow-x-auto rounded border border-white/30 bg-gray-900 shadow">
      <table className="min-w-full border-collapse">
        <thead>
          <tr className="bg-black">
            <th className="border border-white/30 px-4 py-2 text-center text-lg text-gray-200">Rank</th>
            {/* Bandierine inline: nessuna colonna 'Country' */}
        ...
        <tbody>
          {list.map((r) => (
            <tr key={r.year} className="hover:bg-gray-800 border-b border-white/10">
              <td className="border border-white/10 px-4 py-2 text-center text-gray-200 font-semibold">
                {r.rank}
              </td>

              {/* No.1 con bandierina (flagEmoji + iocToIso2) */}
              ...
              {/* No.2 con bandierina (flagEmoji + iocToIso2) */}
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl text-gray-100 font-semibold mb-3">
        Year‑End Difference Between No. 1 and No. 2
      </h2>
      ...
      <Modal show={showModal} onClose={() => setShowModal(false)} title={`Full Year‑End Ranking Gaps`}>
        {renderTable(rows)}
      </Modal>
    </section>
  );
