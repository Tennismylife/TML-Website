...
export default function EoyRankTimespan() {
  const [rank, setRank] = useState<number>(1);
  const [rows, setRows] = useState<EoyRankTimespanItem[]>([]);
  const [loading, setLoading] = useState(false);
  const [page, setPage] = useState(1);
  const [modalItem, setModalItem] = useState<EoyRankTimespanItem[] | null>(null);
  const perPage = 20;

  const fetchRows = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/recordsranking/timespanendoftheseason/count?rank=${rank}`);
      const data: EoyRankTimespanItem[] = await res.json();
      setRows(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error("Error fetching EOY rank-timespan:", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const renderTable = (list: EoyRankTimespanItem[], startIndex = 0) => (
