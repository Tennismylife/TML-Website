// app/api/recordsranking/eoy-top-timespan/route.ts
...
export async function GET(req: Request) {
  try {
    ...
    // 1) Ricava anni disponibili (con eventuali bound) dalle RankingDate
    const dateWhere: any = {};
    ...
    // 2) Ultima data (EOY) per ciascun anno
    ...
    // 4) Aggrega per player → firstYear / lastYear
    ...
    // 5) Output: calcola spanYears, ESCLUDE sempre span=0, ordina per span desc
    const data = Array.from(byPlayer.entries())
      ...
      .filter(row => row.spanYears > 0) // <-- esclusione richiesta
      .sort(
        (a, b) =>
    ...
    console.error("Error computing EOY Top-X timespan:", error);
