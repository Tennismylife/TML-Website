// app/api/recordsranking/oldest-eoy-top/route.ts
import { NextResponse } from "next/server";
import { prisma } from "@/lib/prisma";

// Differenza Y/M/D in UTC (calendario)
...
export async function GET(req: Request) {
  try {
    ...
    // 1) Anni disponibili (bound su RankingDate.date in UTC)
    const dateWhere: any = {};
    ...
    // 2) Last ranking date (EOY) per anno
    ...
    // 4) Per player: occorrenza con età MASSIMA a EOY Top X
    type MaxRec = {
      name: string;
      ioc: string | null;
      year: number;
    ...
    for (const r of rows) {
      ...
      if (ref < birth) continue; // protezione
    ...
    // 5) Output: SOLO anno, ordinato per età decrescente
    ...
    console.error("Error computing oldest EOY in Top-X:", error);
