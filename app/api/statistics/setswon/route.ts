...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const w of winners) {
      ...
      const current = map.get(id) ?? { id, name: "", ioc: "", matches: 0, setsWonTotal: 0 };
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      .sort((a, b) => b.setsWonTotal - a.setsWonTotal)
      .slice(0, top)
      .map(({ id, name, ioc, matches, setsWonTotal }) => ({
        id, name, ioc, matches, output: setsWonTotal,
      }));
