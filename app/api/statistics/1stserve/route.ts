...
type PlayerAggInternal = {
  ...
  svptTotal: number;    // somma w_svpt + l_svpt
};
...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // 1st serve % ponderata
};
  ...
  try {
    ...
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        ...
        _sum: { w_1stIn: true, w_svpt: true },
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
        where,
        _count: { loser_id: true },
        _sum: { l_1stIn: true, l_svpt: true },
      }),
    ]);
    ...
    for (const w of winners) {
      ...
      current.svptTotal += w._sum.w_svpt ?? 0;
      map.set(id, current);
    }
    ...
    for (const l of losers) {
      ...
      current.svptTotal += l._sum.l_svpt ?? 0;
      map.set(id, current);
    }
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      .map(({ id, name, ioc, matches, firstInTotal, svptTotal }) => ({
        ...
        ioc,
        matches,
        output: svptTotal > 0 ? Number(((firstInTotal / svptTotal) * 100).toFixed(3)) : 0,
      }))
