...
type PlayerAggInternal = {
  ...
  ioc: string;
  matches: number;
  wonTotal: number;    // punti vinti (servizio + risposta)
  playedTotal: number; // punti giocati (w_svpt + l_svpt)
};
...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // total points won % (ponderata)
};
  ...
  try {
    ...
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        ...
        _sum: {
          w_svpt: true, w_1stWon: true, w_2ndWon: true,
          l_svpt: true, l_1stWon: true, l_2ndWon: true,
        },
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
    ...
    for (const w of winners) {
      ...
      const w_svpt = w._sum.w_svpt ?? 0;
      const l_svpt = w._sum.l_svpt ?? 0;
      const played = w_svpt + l_svpt;

      const wonService = (w._sum.w_1stWon ?? 0) + (w._sum.w_2ndWon ?? 0);
      const oppWonOnServe = (w._sum.l_1stWon ?? 0) + (w._sum.l_2ndWon ?? 0);
      const wonReturn = Math.min(Math.max(0, l_svpt - oppWonOnServe), l_svpt);
      const won = wonService + wonReturn;
    ...
    for (const l of losers) {
      ...
      const w_svpt = l._sum.w_svpt ?? 0;
      const l_svpt = l._sum.l_svpt ?? 0;
      const played = w_svpt + l_svpt;

      const wonService = (l._sum.l_1stWon ?? 0) + (l._sum.l_2ndWon ?? 0);
      const oppWonOnServe = (l._sum.w_1stWon ?? 0) + (l._sum.w_2ndWon ?? 0);
      const wonReturn = Math.min(Math.max(0, w_svpt - oppWonOnServe), w_svpt);
      const won = wonService + wonReturn;
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      .map(({ id, name, ioc, matches, wonTotal, playedTotal }) => ({
        ...
        ioc,
        matches,
        output: playedTotal > 0 ? Number(((wonTotal / playedTotal) * 100).toFixed(decimals)) : 0,
      }))
