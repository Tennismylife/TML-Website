...
type PlayerAggInternal = {
  ...
  ioc: string;
  matches: number;
  firstWonTotal: number; // somma w_1stWon + l_1stWon
  firstInTotal: number;  // somma w_1stIn + l_1stIn
};
...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // 1st serve won % ponderata
};
  ...
  try {
    ...
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        ...
        _sum: { w_1stWon: true, w_1stIn: true },
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
        where,
        _count: { loser_id: true },
        _sum: { l_1stWon: true, l_1stIn: true },
      }),
    ]);
    ...
    for (const w of winners) {
      ...
      current.firstWonTotal += w._sum.w_1stWon ?? 0;
      current.firstInTotal += w._sum.w_1stIn ?? 0;
      map.set(id, current);
    }
    ...
    for (const l of losers) {
      ...
      current.firstWonTotal += l._sum.l_1stWon ?? 0;
      current.firstInTotal += l._sum.l_1stIn ?? 0;
      map.set(id, current);
    }
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      .map(({ id, name, ioc, matches, firstWonTotal, firstInTotal }) => ({
        id,
        name,
        ioc,
        matches,
        output: firstInTotal > 0 ? Number(((firstWonTotal / firstInTotal) * 100).toFixed(3)) : 0,
      }))
