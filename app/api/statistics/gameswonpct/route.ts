...
function parseWhere(url: URL): Prisma.mVStatsWhereInput {
  ...
  if (statsParam === "true") where.stats = true;
  return where;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ✅ Query in parallelo sulla MV (solo w_gmsWon e l_gmsWon)
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        by: ["winner_id"],
        where,
        _count: { winner_id: true },
        _sum: { w_gmsWon: true, l_gmsWon: true },
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
        where,
    ...
    for (const w of winners) {
      ...
      const gamesPlayed = (w._sum.w_gmsWon ?? 0) + (w._sum.l_gmsWon ?? 0);

      const current = map.get(id) ?? { id, name: "", ioc: "", matches: 0, gamesWonTotal: 0, gamesPlayedTotal: 0 };
      current.matches += w._count.winner_id ?? 0;
      current.gamesWonTotal += gamesWon;
      current.gamesPlayedTotal += gamesPlayed;
      map.set(id, current);
    }
    ...
    for (const l of losers) {
      ...
      const gamesPlayed = (l._sum.w_gmsWon ?? 0) + (l._sum.l_gmsWon ?? 0);
