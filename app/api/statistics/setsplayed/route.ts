...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // total sets played
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    // ✅ Query parallele sulla MV (solo w_setsWon e l_setsWon)
    ...
    for (const w of winners) {
      ...
      const current = map.get(id) ?? { id, name: "", ioc: "", matches: 0, setsPlayed: 0 };
      current.matches += w._count.winner_id ?? 0;
      current.setsPlayed += setsPlayed;
      map.set(id, current);
    }
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      ...
      .slice(0, top)
      .map(({ id, name, ioc, matches, setsPlayed }) => ({
        id, name, ioc, matches, output: setsPlayed,
      }));
