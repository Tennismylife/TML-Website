...
export async function GET(request: NextRequest) {
  try {
    ...
    // ✅ Query parallele sulla MV (tiebreak vinti e giocati)
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        by: ["winner_id"],
        where,
        _count: { winner_id: true },
        _sum: { w_tbWon: true, l_tbWon: true },
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
        where,
    ...
    for (const w of winners) {
      ...
      const tbPlayed = (w._sum.w_tbWon ?? 0) + (w._sum.l_tbWon ?? 0);

      const current = map.get(id) ?? { id, name: "", ioc: "", matches: 0, tiebreaksPlayed: 0, tiebreaksWon: 0 };
      current.matches += w._count.winner_id ?? 0;
      current.tiebreaksWon += tbWon;
      current.tiebreaksPlayed += tbPlayed;
      map.set(id, current);
    }
    ...
    for (const l of losers) {
      ...
      const tbPlayed = (l._sum.w_tbWon ?? 0) + (l._sum.l_tbWon ?? 0);
    ...
    // Calcolo % tiebreak vinti e ordinamento
    const result: PlayerAggResponse[] = Array.from(map.values())
      .map(({ id, name, ioc, matches, tiebreaksPlayed, tiebreaksWon }) => ({
        id,
        name,
        ioc,
        matches,
        output: tiebreaksPlayed > 0 ? Number(((tiebreaksWon / tiebreaksPlayed) * 100).toFixed(decimals)) : 0,
      }))
