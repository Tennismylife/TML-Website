...
type PlayerAggInternal = {
  ...
  ioc: string;
  matches: number;
  firstReturnWonTotal: number; // somma (opponent_1stIn - opponent_1stWon)
  firstFacedTotal: number;     // somma opponent_1stIn
};
...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // 1st return won % (ponderata)
};
  ...
  try {
    ...
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        ...
        _sum: { l_1stIn: true, l_1stWon: true }, // dati dell'avversario
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
        where,
        _count: { loser_id: true },
        _sum: { w_1stIn: true, w_1stWon: true }, // dati dell'avversario
      }),
    ]);
    ...
    for (const w of winners) {
      ...
      if (!id) continue;
      const faced = w._sum.l_1stIn ?? 0;
      const oppWonOnFirst = w._sum.l_1stWon ?? 0;
      const returnWon = Math.min(Math.max(0, faced - oppWonOnFirst), faced);
      ...
      const current = map.get(id) ?? {
        ...
        ioc: "",
        matches: 0,
        firstReturnWonTotal: 0,
        firstFacedTotal: 0,
      };
      current.matches += w._count.winner_id ?? 0;
      current.firstReturnWonTotal += returnWon;
      current.firstFacedTotal += faced;
      map.set(id, current);
    }
    ...
    for (const l of losers) {
      ...
      if (!id) continue;
      const faced = l._sum.w_1stIn ?? 0;
      const oppWonOnFirst = l._sum.w_1stWon ?? 0;
      const returnWon = Math.min(Math.max(0, faced - oppWonOnFirst), faced);
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      .map(({ id, name, ioc, matches, firstReturnWonTotal, firstFacedTotal }) => ({
        id,
        name,
        ioc,
        matches,
        output: firstFacedTotal > 0 ? Number(((firstReturnWonTotal / firstFacedTotal) * 100).toFixed(decimals)) : 0,
      }))
