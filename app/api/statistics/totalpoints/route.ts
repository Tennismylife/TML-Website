...
type PlayerAggInternal = {
  ...
  ioc: string;
  matches: number;
  totalPoints: number; // somma (w_svpt + l_svpt)
};
...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // total points played
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const w of winners) {
      ...
      const matchPointsSum = w_svpt + l_svpt;
      ...
      current.totalPoints += matchPointsSum;
      map.set(id, current);
    }
    ...
    // Ordina per totalPoints e prendi top N; mappa su output
    const result: PlayerAggResponse[] = Array.from(map.values())
      .sort((a, b) => b.totalPoints - a.totalPoints)
      .slice(0, top)
      .map(({ id, name, ioc, matches, totalPoints }) => ({
        id,
        name,
        ioc,
        matches,
