...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // tiebreaks played
};
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const w of winners) {
      ...
      const current = map.get(id) ?? { id, name: "", ioc: "", matches: 0, tiebreaksPlayed: 0 };
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      .sort((a, b) => b.tiebreaksPlayed - a.tiebreaksPlayed)
      .slice(0, top)
      .map(({ id, name, ioc, matches, tiebreaksPlayed }) => ({
        id, name, ioc, matches, output: tiebreaksPlayed,
      }));
