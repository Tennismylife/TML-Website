...
type PlayerAggInternal = {
  ...
  ioc: string;
  matches: number;
  secondReturnWonTotal: number; // somma (opponent second faced - opponent 2nd won)
  secondFacedTotal: number;     // somma opponent second faced
};
...
type PlayerAggResponse = {
  ...
  ioc: string;
  matches: number;
  output: number; // 2nd return points won % (ponderata)
};
  ...
  try {
    ...
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        ...
        _sum: { l_svpt: true, l_1stIn: true, l_2ndWon: true }, // dati dell'avversario
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
        where,
        _count: { loser_id: true },
        _sum: { w_svpt: true, w_1stIn: true, w_2ndWon: true }, // dati dell'avversario
      }),
    ]);
    ...
    for (const w of winners) {
      ...
      if (!id) continue;
      const faced = Math.max(0, (w._sum.l_svpt ?? 0) - (w._sum.l_1stIn ?? 0));
      const oppSecondWon = w._sum.l_2ndWon ?? 0;
      const returnWon = Math.min(Math.max(0, faced - oppSecondWon), faced);
      ...
      const current = map.get(id) ?? {
        ...
        ioc: "",
        matches: 0,
        secondReturnWonTotal: 0,
        secondFacedTotal: 0,
      };
      current.matches += w._count.winner_id ?? 0;
      current.secondReturnWonTotal += returnWon;
      current.secondFacedTotal += faced;
      map.set(id, current);
    }
    ...
    for (const l of losers) {
      ...
      if (!id) continue;
      const faced = Math.max(0, (l._sum.w_svpt ?? 0) - (l._sum.w_1stIn ?? 0));
      const oppSecondWon = l._sum.w_2ndWon ?? 0;
      const returnWon = Math.min(Math.max(0, faced - oppSecondWon), faced);
    ...
    const result: PlayerAggResponse[] = Array.from(map.values())
      .map(({ id, name, ioc, matches, secondReturnWonTotal, secondFacedTotal }) => ({
        id,
        name,
        ioc,
        matches,
        output: secondFacedTotal > 0 ? Number(((secondReturnWonTotal / secondFacedTotal) * 100).toFixed(decimals)) : 0,
      }))
