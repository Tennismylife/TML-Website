...
export async function GET(request: NextRequest) {
  try {
    ...
    const minSetsPlayedParam = url.searchParams.get("minSetsPlayed");
    const minSetsPlayed = minSetsPlayedParam && !Number.isNaN(Number(minSetsPlayedParam)) ? Number(minSetsPlayedParam) : 0;

    // ✅ Query parallele sulla MV
    const [winners, losers] = await Promise.all([
      prisma.mVStats.groupBy({
        by: ["winner_id"],
        where,
        _count: { winner_id: true },
        _sum: { w_setsWon: true, l_setsWon: true },
      }),
      prisma.mVStats.groupBy({
        by: ["loser_id"],
        where,
    ...
    for (const w of winners) {
      ...
      const setsWon = w._sum.w_setsWon ?? 0;
      const setsPlayed = (w._sum.w_setsWon ?? 0) + (w._sum.l_setsWon ?? 0);

      const current = map.get(id) ?? { id, name: "", ioc: "", matches: 0, setsWonTotal: 0, setsPlayedTotal: 0 };
      current.matches += w._count.winner_id ?? 0;
      current.setsWonTotal += setsWon;
      current.setsPlayedTotal += setsPlayed;
      map.set(id, current);
    }
    ...
    for (const l of losers) {
      ...
      const setsWon = l._sum.l_setsWon ?? 0;
      const setsPlayed = (l._sum.w_setsWon ?? 0) + (l._sum.l_setsWon ?? 0);
    ...
    // Calcolo % set vinti e filtro su minSetsPlayed
    const result: PlayerAggResponse[] = Array.from(map.values())
      .filter(r => r.setsPlayedTotal >= minSetsPlayed)
      .map(({ id, name, ioc, matches, setsWonTotal, setsPlayedTotal }) => ({
        id,
        name,
        ioc,
        matches,
        output: setsPlayedTotal > 0 ? Number(((setsWonTotal / setsPlayedTotal) * 100).toFixed(decimals)) : 0,
      }))
