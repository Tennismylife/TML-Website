...
export async function GET(request: NextRequest) {
  try {
    ...
    const where: any = {
      AND: [
        { NOT: [{ score: { contains: 'W/O' } }, { score: { contains: 'WEA' } }, { score: { contains: 'RET' } }] },
      ],
    };
    ...
    else where.AND.push({ best_of: 5 }); // default to 5 if not specified
    ...
    for (const m of matches) {
      ...
      let loserSetCount = 0;
      let up2to1: 'winner' | 'loser' | null = null;

      for (const setScore of sets) {
        const cleanedSet = setScore.replace(/\(.*?\)/g, ''); // rimuove tie-break
        if (!cleanedSet.includes('-')) continue;

        const [a, b] = cleanedSet.split('-').map(Number);
        if (Number.isNaN(a) || Number.isNaN(b)) continue;

        if (a > b) winnerSetCount++;
        else loserSetCount++;

        if (winnerSetCount === 2 && loserSetCount === 1) up2to1 = 'winner';
        if (loserSetCount === 2 && winnerSetCount === 1) up2to1 = 'loser';
      }
