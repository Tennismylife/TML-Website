...
export async function GET(request: NextRequest) {
  try {
    ...
    type Row = { id: string; name: string; ioc: string; slug: string | null; wins_at_age: bigint };
    let rows: Row[];
    ...
    if (filtersCount <= 1) {
      ...
      rows = await prisma.$queryRaw<Row[]>(Prisma.sql`
        ...
        WHERE ${ageCondition}
        GROUP BY m.winner_id, p.player, p.ioc, p.slug
        ORDER BY wins_at_age DESC, name ASC
        LIMIT ${limit}
      `);
    } else {
      const filters: Prisma.Sql[] = [
        Prisma.sql`m.status = true`, Prisma.sql`m.winner_id IS NOT NULL`, Prisma.sql`m.winner_age IS NOT NULL`,
        after ? Prisma.sql`m.winner_age >= ${targetAge}` : Prisma.sql`m.winner_age <= ${targetAge}`,
      ];
    ...
    return NextResponse.json(rows.map(r => ({ ...r, id: String(r.id), wins_at_age: Number(r.wins_at_age) })));
  } catch (error) {
    console.error('[GET /api/records/atage/wins] Error:', error);
