...
export async function GET(request: NextRequest) {
  try {
    ...
    const common: Prisma.Sql[] = [Prisma.sql`m.team_event = false`];
    ...
    type Row = { id: string; name: string; ioc: string; slug: string | null; participations_at_age: bigint };
    const rows = await prisma.$queryRaw<Row[]>(Prisma.sql`
      WITH appearances AS (
        SELECT m.winner_id AS player_id, m.event_id
        FROM "Match" m
        WHERE ${Prisma.join(common, ' AND ')}
          AND m.winner_id IS NOT NULL AND m.event_id IS NOT NULL
          AND m.winner_age IS NOT NULL AND ${winnerAge}
        UNION ALL
        SELECT m.loser_id AS player_id, m.event_id
        FROM "Match" m
        WHERE ${Prisma.join(common, ' AND ')}
          AND m.loser_id IS NOT NULL AND m.event_id IS NOT NULL
          AND m.loser_age IS NOT NULL AND ${loserAge}
      ), agg AS (
        SELECT player_id, COUNT(DISTINCT event_id) AS participations_at_age
        FROM appearances
        GROUP BY player_id
      )
      SELECT a.player_id AS id,
             ...
             p.slug,
             a.participations_at_age
      FROM agg a
      JOIN "Player" p ON p.id = a.player_id
      ORDER BY a.participations_at_age DESC, name ASC
      LIMIT ${limit}
    `);
    ...
    return NextResponse.json(rows.map(r => ({
      ...
      ioc: r.ioc,
      slug: r.slug,
      participations_at_age: Number(r.participations_at_age),
    })));
  } catch (error) {
    console.error('[GET /api/records/atage/entries] Error:', error);
