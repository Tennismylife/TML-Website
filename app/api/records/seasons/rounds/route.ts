...
export async function GET(request: NextRequest) {
  try {
    ...
    if (selectedSurfaces.length + selectedLevels.length + selectedRounds.length >= 2) {
      ...
      matches.forEach(m => {
        ...
        [m.winner_id, m.loser_id].forEach(pid => {
          ...
          if (!groupedMap[key]) groupedMap[key] = { player_id: String(pid), year: year, total_rounds: 0 };
    ...
    const ids = Array.from(new Set(finalRounds.map(p => String(p.player_id)))).filter(Boolean);
    if (ids.length > 0) {
