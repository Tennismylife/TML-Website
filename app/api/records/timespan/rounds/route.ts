...
export async function GET(request: NextRequest) {
  try {
    ...
    const ids = topRows.map(r => String(r.id));
    if (ids.length > 0) {
      ...
      const enriched = topRows.map(r => ({ ...r, slug: slugMap.get(String(r.id)) ?? null }));
      return NextResponse.json({ data: enriched, round: roundParam });
    }
