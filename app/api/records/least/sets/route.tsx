...
// ✅ Gestione tie-break e super tie-break per set persi
function parseSetsLost(score: string): number {
  ...
  for (const set of sets) {
    ...
    if (parts.length === 2) {
      const winnerGames = parseInt(parts[0], 10);
      const loserGames = parseInt(parts[1], 10);
      if (!isNaN(winnerGames) && !isNaN(loserGames) && loserGames > winnerGames) {
        setsLost += 1;
      }
    }
  }
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Ordino per meno set persi
    records.sort((a, b) => a.setsLost - b.setsLost);
