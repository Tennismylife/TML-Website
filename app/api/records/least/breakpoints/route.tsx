...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const [tId, data] of tournamentWinners) {
      ...
      for (const m of matches) {
        if (String(m.winner_id) === winnerId) {
          ...
          totalBreakPointsFaced += m.w_bpFaced || 0;
          matchList.push({
            opponent: m.loser_name,
            date: m.tourney_date,
            tournament: data.name,
            round: m.round,
            breakPointsFaced: m.w_bpFaced || 0,
          });
        }
      }
      ...
      if (hasValues) {
        records.push({
          player: data.winner,
          breakPointsFaced: totalBreakPointsFaced,
          tournament: data.name,
    ...
    // Ordino per meno break point faced
    records.sort((a, b) => a.breakPointsFaced - b.breakPointsFaced);
