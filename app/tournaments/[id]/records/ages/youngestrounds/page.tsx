...
export default async function Page({ params }: any) {
  ...
  // SSR data fetch for ages/youngestrounds table
  let agesData: any;
  try {
    const res = await fetch(`${site}/api/tournaments/${id}/records/ages/youngestrounds`, { cache: 'no-store' });
  ...
  return (
    <div>
      <main className="w-full mx-auto text-white relative">
        ...
        <RecordsBreadcrumb slugId={slugId} tournamentName={tournamentName} crumbs={[{ label: 'Ages', href: `/tournaments/${slugId}/records/ages/main` }, { label: 'Youngest by Round' }]} className="px-2" />
