...
export default async function Page({ params }: any) {
  ...
  return (
    <div className="w-full mx-auto text-white relative">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({ '@context': 'https://schema.org', '@type': 'BreadcrumbList', itemListElement: [{ '@type': 'ListItem', position: 1, name: 'Home', item: `${site}/` }, { '@type': 'ListItem', position: 2, name: 'Tournaments', item: `${site}/tournaments` }, { '@type': 'ListItem', position: 3, name: tournamentName, item: `${site}/tournaments/${slugId}` }, { '@type': 'ListItem', position: 4, name: 'Records', item: `${site}/tournaments/${slugId}/records` }, { '@type': 'ListItem', position: 5, name: 'Ages', item: `${site}/tournaments/${slugId}/records/ages/main` }, { '@type': 'ListItem', position: 6, name: 'Youngest by Round', item: `${site}/tournaments/${slugId}/records/ages/youngestrounds` }, { '@type': 'ListItem', position: 7, name: String(title).toUpperCase(), item: `${site}/tournaments/${slugId}/records/ages/youngestrounds/${encodeURIComponent(String(title))}` }] }) }} />
      <ViewRecordsCTA id={id} className="absolute top-4 left-4 z-50" />
      <div className="pt-14 px-2">
        <RecordsBreadcrumb slugId={slugId} tournamentName={tournamentName} crumbs={[{ label: 'Ages', href: `/tournaments/${slugId}/records/ages/main` }, { label: 'Youngest by Round', href: `/tournaments/${slugId}/records/ages/youngestrounds` }, { label: String(title).toUpperCase() }]} />
      </div>
      <AgesFull id={id} section="youngestrounds" title={title} />
    </div>
  );
}
