...
export default async function SEOPlayer({ playerId, slug, name, atpname, tab = 'overview' }: SEOPlayerProps) {
 
  const siteUrl = process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000';
  const url = `${siteUrl}/players/${slug}${tab !== 'overview' ? `?tab=${tab}` : ''}`;

  // === Fetch dei match dal database o API ===
  const allMatches: any[] = await fetch(`${siteUrl}/api/players/allmatches?id=${playerId}`).then(r => r.json());
  const matches = allMatches.filter((m) => m.status !== false);

  const totalMatches = matches.length;
  const careerWins = matches.filter((m) => m.winner_id === playerId).length;
  const careerLosses = totalMatches - careerWins;

  // Wins per surface
  const clayWins = matches.filter((m) => m.winner_id === playerId && m.surface === 'Clay').length;
  const hardWins = matches.filter((m) => m.winner_id === playerId && m.surface === 'Hard').length;
  const grassWins = matches.filter((m) => m.winner_id === playerId && m.surface === 'Grass').length;

  const clayLosses = matches.filter((m) => m.loser_id === playerId && m.surface === 'Clay').length;
  const hardLosses = matches.filter((m) => m.loser_id === playerId && m.surface === 'Hard').length;
  const grassLosses = matches.filter((m) => m.loser_id === playerId && m.surface === 'Grass').length;
  ...
  matches.filter((m) => m.winner_id === playerId && m.round === 'F')
    .forEach((m) => {
  ...
  const jsonLd: any = {
    ...
    description: `Profile of ${atpname}.`,
    additionalProperty: [
