...
export async function generateMetadata({ searchParams }: { searchParams?: Promise<Record<string, string | string[] | undefined>> }): Promise<Metadata> {
  ...
  const title = `Youngest Year-End ATP Top ${top} – Age Records`;
  const description = `Who are the youngest players to finish a season inside the ATP Top ${top}? All-time historical list ordered by age.`;
  const canonical = `${SITE}/recordsranking/agesendoftheseason/youngestattop/${top}`;
  return {
    title,
    description,
    keywords: [`youngest year-end Top ${top}`, 'ATP year-end age records', 'youngest year-end ATP', 'tennis records by age', 'ATP history'],
...
export default async function YoungestEoyTop({ searchParams }: { searchParams?: Promise<Record<string, string | string[]>> }) {
  ...
  // Reuse the API logic server-side
  // 1) years
  ...
  for (const r of rowsData) {
    ...
    if (!prev || ageDays < prev.ageDays || (ageDays === prev.ageDays && ref < prev.date)) {
      bestByPlayer.set(id, { name: r.player?.atpname ?? '', ioc: r.player?.ioc ?? null, year: recYear, date: ref, birth, ageDays, slug: r.player?.slug ?? null });
    }
  }

  const data: YoungestEoyTopItem[] = Array.from(bestByPlayer.entries()).map(([id, v]) => {
      ...
      <table className="min-w-full border-collapse">
        ...
        <thead>
          <tr className="bg-black">
            <th className="border border-white/30 px-4 py-2 text-center text-lg text-gray-200">Top</th>
            <th className="border border-white/30 px-4 py-2 text-center text-lg text-gray-200">Player</th>
            <th className="border border-white/30 px-4 py-2 text-center text-lg text-gray-200">Age at EOY Top {top}</th>
  ...
  return (
    <section className="mb-8">
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify({
        ...
        'url': `https://stats.tennismylife.org/recordsranking/agesendoftheseason/youngestattop/${top}`,
        'inLanguage': 'en-US',
        'isPartOf': { '@type': 'WebSite', 'name': 'TennisMyLife', 'url': 'https://stats.tennismylife.org' },
        'dateModified': new Date().toISOString(),
        'name': `Youngest Year-End ATP Top ${top} Players – All-Time`,
        'description': `Players who finished a season inside the ATP Top ${top} at the youngest age ever.`,
      ...
      {page === 1 && data.length > 0 && (
        <div className="mb-6 px-5 py-4 rounded-xl bg-gray-800/50 border border-white/10 text-gray-400 text-sm leading-relaxed max-w-3xl mx-auto">
          The youngest player to finish a season inside the ATP Top{' '}<span className="text-white font-medium">{top}</span> is{' '}
