...
export default function YoungestEoyAtRank() {
  const searchParams = useSearchParams();
  const router = useRouter();
  const pathname = usePathname();

  const initialRank = Number(searchParams?.get('rank') ?? 1);
  const [rank, setRank] = useState<number>(initialRank);
  const [rows, setRows] = useState<YoungestEoyItem[]>([]);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [showModal, setShowModal] = useState(false);
  const perPage = 20;

  // Keep URL in sync when user changes rank
  useEffect(() => {
    if (!pathname) return;
  ...
  const fetchRows = async (selectedRank: number) => {
    ...
    try {
      const res = await fetch(
        `/api/recordsranking/agesendoftheseason/youngestcount?rank=${selectedRank}&limit=200`
      );
      const data = await res.json();
      setRows(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error("Error fetching youngest EOY:", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section className="mb-8">
      ...
      <h2 className="text-xl font-semibold mb-4 text-gray-200 text-center">
        Youngest Players at Year-End No. {rank}
      </h2>
