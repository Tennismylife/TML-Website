  ...
  const fetchRows = async (selectedRank: number) => {
    ...
    try {
      const res = await fetch(
        `/api/recordsranking/ages/youngestcount?rank=${selectedRank}&limit=200`
      );
      const data = await res.json();
      setRows(Array.isArray(data) ? data : []);
    } catch (err) {
      console.error("Error fetching youngest list:", err);
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section className="mb-8">
      ...
      <h2 className="text-xl font-semibold mb-4 text-gray-200 text-center">
        Youngest Players to Reach No. {rank}
      </h2>
