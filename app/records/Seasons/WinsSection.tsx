...
export default function WinsSection({ selectedSurfaces, selectedLevels, selectedRounds, selectedBestOf }: WinsSectionProps) {
  ...
  if (!topSameTournamentWins.length) return <div className="text-center py-8 text-gray-300">No wins found.</div>;
            ...
            return (
              <tr key={`${p.winner_id}-${p.year}-${idx}`} className="hover:bg-gray-800 border-b border-white/10">
                ...
                <td className="border border-white/10 px-4 py-2 flex items-center gap-2 text-gray-200">
                  ...
                  <Link href={getPlayerLink(p.winner_id)} className="text-indigo-300 hover:underline">{p.player_name}</Link>
                </td>
                <td className="border border-white/10 px-4 py-2 text-center text-gray-200">{p.total_wins}</td>
                <td className="border border-white/10 px-4 py-2 text-center text-indigo-300">{p.year}</td>
              </tr>
            );
          })}
        </tbody>
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl font-semibold mb-4 text-gray-200">Top Wins in the Same Season</h2>
