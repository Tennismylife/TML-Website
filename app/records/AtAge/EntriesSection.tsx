...
export default function EntriesSection({ selectedSurfaces, selectedLevels }: EntriesSectionProps) {
      ...
      <table className="min-w-full border-collapse">
        <thead>
          <tr className="bg-black">
            ...
            <th className="border border-white/30 px-4 py-2 text-center text-lg text-gray-200">Participations at {selectedAge.toFixed(3)}</th>
          </tr>
        </thead>
        <tbody>
          {players.map((p, idx) => {
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl font-semibold mb-4 text-gray-200">Top Tournament Participations at Age</h2>
      ...
      <Modal
        ...
        title={`Top Tournament Participations at Age ${selectedAge.toFixed(3)}`}
      >
        {renderTable(data)}
      </Modal>
    </section>
