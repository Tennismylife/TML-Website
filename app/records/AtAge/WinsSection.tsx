...
export default function WinsSection({ selectedSurfaces, selectedLevels, selectedRounds, selectedBestOf }: WinsSectionProps) {
  ...
  useEffect(() => {
    ...
    const url = `/api/records/atage/wins?${query.toString()}`;
    setLoading(true);
  ...
  const filteredData = useMemo(() => {
    return data
      .map(player => {
        const ages = player.ages?.ages_json || {};
        const agesArray = Object.entries(ages)
          .map(([ageStr, count]) => [parseFloat(ageStr), Number(count)] as [number, number])
          .sort((a, b) => a[0] - b[0]);

        let cumulativeWins = 0;
        for (const [age, wins] of agesArray) {
          if (age <= maxAge) cumulativeWins = wins;
          else break;
        }

        return { ...player, count: cumulativeWins };
      })
      .filter(p => p.count > 0)
    ...
    <section className="mb-8">
      ...
      <div className="mb-4 flex gap-4 items-center">
        <label className="flex items-center gap-2 flex-1 text-gray-200">
          Max Age for Wins: {years}y {days}d
          <input
            type="range"
            min={0}
            max={50*365}
