...
interface SetRecord {
  ...
  year: number;
  date: string;
  matches: { opponent: string; date: string; tournament: string; round: string; setsLost: number }[];
}

export default function SetsSection({ selectedSurfaces, selectedLevels }: SetsSectionProps) {
  ...
  useEffect(() => {
    ...
    const url = `/api/records/least/sets${query.toString() ? '?' + query.toString() : ''}`;
  ...
  const { records: setsArray } = data || {};

  const SubModal = ({ show, onClose, matches }: { show: boolean; onClose: () => void; matches: SetRecord['matches'] }) => {
    if (!show) return null;
    return (
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b">
                ...
                <th className="text-left py-1">Sets Lost</th>
              </tr>
            </thead>
            <tbody>
              {matches.map((m, index) => (
                <tr key={index} className="border-b">
                  ...
                  <td className="py-1">{m.setsLost}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <div>
      <h3 className="text-md font-semibold mb-2">Sets Lost to Win a Title</h3>
      <table className="w-full text-sm border-collapse">
        <thead>
          <tr className="border-b">
        ...
        <tbody>
          {setsArray && setsArray.length === 0 ? (
            <tr>
              <td colSpan={4} className="py-2 text-gray-500">No data</td>
            </tr>
          ) : (
            setsArray && setsArray.map((p: SetRecord, index: number) => (
              <tr key={index} className="border-b">
                <td className="py-1">
                ...
                <td className="py-1">{p.setsLost}</td>
