...
export default function MinutesSection({ selectedSurfaces, selectedLevels }: MinutesSectionProps) {
  ...
  useEffect(() => {
    ...
    const url = `/api/records/least/minutes${query.toString() ? '?' + query.toString() : ''}`;
    fetch(url)
      .then(res => res.json())
      .then(setData)
      .catch(setError)
  ...
  const renderTable = (records: MinutesRecord[]) => (
    <table className="w-full text-sm border-collapse">
      <thead>
        <tr className="border-b">
          <th className="text-left py-1">Player</th>
          <th className="text-left py-1">Total Minutes</th>
        ...
        {records.length === 0 ? (
          ...
          records.map((r, index) => (
            <tr key={index} className="border-b">
              ...
              <td className="py-1">{r.totalMinutes}</td>
  ...
  return (
    <section className="rounded border bg-white p-4">
      <h2 className="text-lg font-bold mb-4">Least Minutes to Win Title</h2>
      {renderTable(recordsArray.slice(0, 10))}
      {recordsArray.length > 10 && (
        <button
          onClick={() => { setModalTitle('Least Minutes to Win Title'); setModalRecords(recordsArray); setShowModal(true); }}
