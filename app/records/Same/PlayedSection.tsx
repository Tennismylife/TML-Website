...
export default function PlayedSection({ selectedSurfaces, selectedLevels, selectedRounds, selectedBestOf }: PlayedSectionProps) {
  const [topSameTournamentPlayed, setTopSameTournamentPlayed] = useState<PlayedRecord[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fetchedData: PlayedRecord[] = await res.json();
        setTopSameTournamentPlayed(fetchedData || []);
      } catch (err) {
        console.error(err);
        setTopSameTournamentPlayed([]);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!topSameTournamentPlayed.length) return <div className="text-center py-8 text-gray-300">No matches found.</div>;

  const totalPages = Math.ceil(topSameTournamentPlayed.length / perPage);
  const start = (page - 1) * perPage;
  const currentData = topSameTournamentPlayed.slice(start, start + perPage);
            ...
            return (
              <tr key={`${p.player_id}-${p.tourney_id}`} className="hover:bg-gray-800 border-b border-white/10">
                ...
                <td className="border border-white/10 px-4 py-2 flex items-center gap-2 text-gray-200">
                  ...
                  <Link href={getLink(String(p.player_id))} className="text-indigo-300 hover:underline">
                    {p.player_name}
                  </Link>
                </td>
                <td className="border border-white/10 px-4 py-2 text-center text-gray-200">{p.total_matches}</td>
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl font-semibold mb-4 text-gray-200">Top Matches Played in the Same Tournament</h2>
