...
export default function WinsSection({ selectedSurfaces, selectedLevels, selectedRounds, selectedBestOf }: WinsSectionProps) {
  const [topSameTournamentWins, setTopSameTournamentWins] = useState<any[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setTopSameTournamentWins(fetchedData);
      } catch (err) {
        console.error(err);
            ...
            return (
              <tr key={`${p.winner_id}-${p.tourney_id}-${idx}`} className="hover:bg-gray-800 border-b border-white/10">
                ...
                <td className="border border-white/10 px-4 py-2 flex items-center gap-2 text-gray-200">
                  ...
                  <Link href={getLink(p.winner_id)} className="text-indigo-300 hover:underline">
                    {p.player_name}
                  </Link>
                </td>
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl font-semibold mb-4 text-gray-200">Top Wins in the Same Tournament</h2>
