  ...
  return (
    <section className="mb-8">
      ...
      {isMostQuarterfinalsAtSingleTournament && (
        <div className="mb-6 p-6 bg-gray-800 rounded-lg shadow-lg text-sm leading-relaxed space-y-3 !text-gray-200">
          <p>
            At the top of the Open Era list for Most Quarterfinals at a Single Tournament stands {renderPlayerLink('Roger Federer', 'roger-federer', 'CHE')}, who reached the <Link href={getTourneyHref({ slug: 'wimbledon' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Wimbledon</Link> quarterfinals <strong className="!text-amber-300">18</strong> times — the highest men’s total at one tournament. Federer’s Wimbledon quarterfinal streak set the Grand Slam benchmark for repeated last-eight appearances.
          </p>
          <p>
            Behind him, the next single-tournament quarterfinal reference is {renderPlayerLink('Jimmy Connors', 'jimmy-connors', 'USA')} at the <Link href={getTourneyHref({ slug: 'us-open' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">US Open</Link> with <strong className="!text-amber-300">17</strong> quarterfinals, followed by {renderPlayerLink('Novak Djokovic', 'novak-djokovic', 'SRB')} with <strong className="!text-amber-300">17</strong> <Link href={getTourneyHref({ slug: 'roland-garros' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Roland Garros</Link> quarterfinals and <strong className="!text-amber-300">17</strong> <Link href={getTourneyHref({ slug: 'rome-masters' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Rome Masters</Link> quarterfinals. Those totals show how Djokovic has matched the most elite single-tournament quarterfinal profiles on both clay majors and the Masters 1000 circuit.
          </p>
          <p>
            Another strong non-Slam cluster sits at <strong className="!text-amber-300">16</strong>: {renderPlayerLink('Roger Federer', 'roger-federer', 'CHE')} at <Link href={getTourneyHref({ slug: 'basel' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Basel / Swiss Indoors</Link> and <Link href={getTourneyHref({ slug: 'halle' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Halle</Link>, and {renderPlayerLink('Rafael Nadal', 'rafael-nadal', 'ESP')} at <Link href={getTourneyHref({ slug: 'rome-masters' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Rome Masters</Link> and <Link href={getTourneyHref({ slug: 'madrid-masters' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Madrid Masters</Link>. Basel still stands out for Federer because it also includes 15 finals at his hometown tournament.
          </p>
          <p>
            In this record, the milestone is not just winning trophies, but repeatedly surviving to the last eight at the same tournament across many years: {renderPlayerLink('Roger Federer', 'roger-federer', 'CHE')} set the overall single-tournament ceiling at <Link href={getTourneyHref({ slug: 'wimbledon' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Wimbledon</Link> with <strong className="!text-amber-300">18</strong> quarterfinals, {renderPlayerLink('Jimmy Connors', 'jimmy-connors', 'USA')} holds the <Link href={getTourneyHref({ slug: 'us-open' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">US Open</Link> reference with <strong className="!text-amber-300">17</strong>, {renderPlayerLink('Novak Djokovic', 'novak-djokovic', 'SRB')} owns the modern clay and Masters records with <strong className="!text-amber-300">17</strong> <Link href={getTourneyHref({ slug: 'roland-garros' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Roland Garros</Link> and <strong className="!text-amber-300">17</strong> <Link href={getTourneyHref({ slug: 'rome-masters' })} className="!text-orange-300 hover:!text-orange-100 font-semibold">Rome</Link> quarterfinals, while the non-Slam group at <strong className="!text-amber-300">16</strong> includes Basel, Halle, Rome and Madrid.
          </p>
        </div>
      )}
