...
export default function EntriesSection({ selectedSurfaces, selectedLevels }: EntriesSectionProps) {
  const [topSameTournamentEntries, setTopSameTournamentEntries] = useState<EntryRecord[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const fetchedData: EntryRecord[] = await res.json();
        setTopSameTournamentEntries(fetchedData || []);
      } catch (err) {
        console.error(err);
        setTopSameTournamentEntries([]);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (!topSameTournamentEntries.length) return <div className="text-center py-8 text-gray-300">No entries found.</div>;

  const totalPages = Math.ceil(topSameTournamentEntries.length / perPage);
  const start = (page - 1) * perPage;
  const currentData = topSameTournamentEntries.slice(start, start + perPage);
            ...
            return (
              <tr key={`${p.player_id}-${p.tourney_id}`} className="hover:bg-gray-800 border-b border-white/10">
                ...
                <td className="border border-white/10 px-4 py-2 flex items-center gap-2 text-gray-200">
                  ...
                  <Link href={getLink(String(p.player_id))} className="text-indigo-300 hover:underline">{p.player_name}</Link>
                </td>
                <td className="border border-white/10 px-4 py-2 text-center text-gray-200">{p.total_entries}</td>
  ...
  return (
    <section className="mb-8">
      <h2 className="text-xl font-semibold mb-4 text-gray-200">Top Entries in the Same Tournament</h2>
