-- materialized view for youngest players reaching top 100
DROP MATERIALIZED VIEW IF EXISTS mv_ages_youngesttop_100;
CREATE MATERIALIZED VIEW mv_ages_youngesttop_100 AS
WITH ranked AS (
    SELECT r."playerId"            AS player_id,
           ...
           row_number() OVER (
               ...
               ORDER BY (rd.date - p.birthdate) ASC, rd.date ASC
           ) AS rn
    FROM "Ranking" r
