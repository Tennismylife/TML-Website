...
console.log('=== TOP ATP 500 MATCHES PLAYED ===');
...
for (const p of top5) {
  const years = await prisma.$queryRaw`
    ...
    FROM (
      SELECT year FROM "Match" WHERE tourney_level = '500' AND winner_id = ${p.player_id}
      UNION ALL
      SELECT year FROM "Match" WHERE tourney_level = '500' AND loser_id = ${p.player_id}
    ) AS player_matches
  `;
