...
console.log('=== TOP DAVIS CUP MATCHES PLAYED ===');
...
for (const p of top5) {
  const years = await prisma.$queryRaw`
    ...
    FROM (
      SELECT year FROM "Match" WHERE tourney_level = 'D' AND winner_id = ${p.player_id}
      UNION ALL
      SELECT year FROM "Match" WHERE tourney_level = 'D' AND loser_id = ${p.player_id}
    ) AS player_matches
  `;
